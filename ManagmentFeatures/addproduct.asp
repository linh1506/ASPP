    ...
    <div class="container">
        <form method="post">
            ...
            <div>
             ...
             <div>
                <label for="isAvailable" class="form-lable">Opening Price:</label>
                <input type="number" name='price' oninput="this.value|=0">
            </div>
            <div>
                <label for="description" class="form-label">Description</label>
                <!--<input type="text" name="description" id="description" class="form-control">-->
                <textarea class="form-control" id="myTextarea" name="description" rows="5"></textarea>
            </div>
        ...
        <%  
            ...
            If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
                ...
                cmdPrep.Execute
                    ...
                    Dim i,IDforSizes,sqlQuery
                    sqlQuery = "Select ID from PRODUCT where NAME ="
                    sqlQuery = sqlQuery & "'" & shoeName & "'"
                    cmdPrep.CommandText = sqlQuery
                    Set Result = cmdPrep.execute 
                    If not Result.EOF Then
                        IDforSizes = Result("ID")
                    End If
                    For i = 0 To UBound(shoeSizes)
                        cmdPrep.commandText="insert into PRODUCT_SIZE values(" & IDforSizes & "," & Cint(shoeSizes(i)) & "," & Cint(shoeQuantities(i)) & ")"
                        cmdPrep.execute
                    Next
